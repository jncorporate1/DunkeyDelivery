...
SELECT
  ...
  COALESCE(t2.Image, t1.Image) AS ImageUrl,
...
";
                    ...
                    return Ok(new CustomResponse<SearchProductListViewModel> { Message = ResponseMessages.Success, StatusCode = (int)HttpStatusCode.OK, Result = new SearchProductListViewModel { Products = packageProducts } });
                }
            }
            catch (Exception ex)
            {
